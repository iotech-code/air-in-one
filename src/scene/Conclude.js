                ...
                <ScrollView>
                    <View style={[styles.container, styles.mt_1]}>
                        ...
                        <View style={[styles.flexRowBetween]} >
                            <TouchableOpacity onPress={() => Actions.push('SelectType')} style={{ ...styles.boxIconCircle }}>
...

                        {
                            selectorCalculate ?
                                <View style={{ marginTop: '30%' }}>
                                    <Text style={{ textAlign: 'center', fontSize: hp('2%'), fontFamily: defaultTheme.FONT.FONT_FAMILY, color: '#215974' }}>จำนวน BTU ที่เหมาะสมกับห้องคุณได้เเก่....</Text>
                                </View>
                                :
                                <View style={{ marginTop: '30%' }}>
                                    <Text style={{ textAlign: 'center', fontSize: hp('2%'), fontFamily: defaultTheme.FONT.FONT_FAMILY, color: '#215974' }}>จำนวน BTU ที่คุณเลือกคือ....</Text>
                                </View>
                        }
                        ...
                        <View style={{ marginTop: 30 }}>
                            <Text style={{ textAlign: 'center', fontSize: hp('3%'), fontWeight: 'bold', color: '#6CC3ED', fontFamily: defaultTheme.FONT.FONT_FAMILY }}>
                                {!selectorCalculate ? btuForAdvice : 'คำนวณ'} BTU
                        </Text>
                        </View>


                        <View style={[styles.flexRowBetween, { padding: hp('2.5%'), marginTop: hp('3%'), backgroundColor: '#F7F7F7', borderRadius: 5 }]}>
                            <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, fontSize: hp('1.8%'), alignSelf: 'center' }}>ประเภทของเเอร์</Text>
                            <View style={{...styles.flexRowEnd,alignItems:'center'}}>
                                {
                                   selectorCalculate ?  this.renderImageAir(roomInfo.type)  :  this.renderImageAir(selectType.air)
                                }
                                <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, color: '#6CC3ED', fontSize: hp('1.8%'), alignSelf: 'center' }}>{selectorCalculate ? roomInfo.type : selectType.air}</Text>
                            </View>
                        </View>
                        ...
                        <View style={[styles.flexRowBetween, { padding: hp('2.5%'), marginTop: hp('1%'), backgroundColor: '#F7F7F7', borderRadius: 5 }]}>
                            ...
                            {selectType.air ? this.renderImageBrand(selectType.brand) : null}
                        </View>

                        <View style={[styles.flexRowBetween, { padding: hp('2.5%'), marginTop: hp('1%'), backgroundColor: '#F7F7F7', borderRadius: 5 }]}>
                            <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, fontSize: hp('1.8%'), alignSelf: 'center' }}>ชนิดของเเอร์</Text>
                            <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, fontSize: hp('1.8%'), alignSelf: 'center', color: '#6CC3ED' }}>{selectorCalculate ? roomInfo.airType : selectType.type}</Text>
                        </View>

                        <View style={[styles.flexRowBetween, { padding: hp('2.5%'), marginTop: hp('1%'), backgroundColor: '#F7F7F7', borderRadius: 5 }]}>
                            <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, fontSize: hp('1.8%'), alignSelf: 'center' }}>ช่วงราคา</Text>
                            <Text style={{ fontFamily: defaultTheme.FONT.FONT_FAMILY, fontSize: hp('1.8%'), alignSelf: 'center', color: '#6CC3ED' }}>{selectorCalculate ? roomInfo.price : selectType.price} บาท</Text>
                        </View>



                        <TouchableOpacity style={[styleScoped.nextButton, styles.mb_1, styles.flexRowCenter, { marginTop: hp('14%') }]} onPress={() => Actions.push('Product')}>
                            <Text style={{ color: 'white', fontSize: hp('2%'), alignSelf: 'center', fontFamily: defaultTheme.FONT.FONT_FAMILY }}>ถัดไป</Text>
                            <Icon name="chevron-right" size={defaultTheme.size.iconOnTop} style={{ color: 'white', alignSelf: 'center' }} />
                        </TouchableOpacity>
